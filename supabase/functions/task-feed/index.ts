...
Deno.serve(async (req) => {
  ...
  const { data: who } = await admin.from("mg_ical_tokens").select("email").eq("token", token).maybeSingle();
  ...
  const [recRes, taskRes, entRes] = await Promise.all([
    admin.from("recurring_tasks").select("*").eq("active", true).ilike("assignee_email", email),
    admin.from("tasks").select("*").ilike("assignee_email", email).neq("status", "done")
  ...
  const lines: string[] = [
    "BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//CallidusCo//Owner Portal//EN",
    "CALSCALE:GREGORIAN", "METHOD:PUBLISH",
    "X-WR-CALNAME:Callidus tasks", "X-WR-TIMEZONE:America/Chicago",
  ];
  ...
  for (const r of recRes.data || []) {
    ...
    lines.push(
      "BEGIN:VEVENT",
      `UID:recur-${r.id}@callidusco.com`,
      `DTSTAMP:${stamp}`,
      `DTSTART:${startLocal}`,
      `DTEND:${endLocal}`,
      `RRULE:${rrule}`,
      `SUMMARY:${esc(r.title + (r.entity_id ? " — " + entName(r.entity_id) : ""))}`,
      `DESCRIPTION:${esc("Recurring task · Callidus Owner Portal — https://callidusco.com/administration")}`,
      "END:VEVENT",
    );
  }
  ...
  for (const t of taskRes.data || []) {
    ...
    lines.push(
      "BEGIN:VEVENT",
      `UID:task-${t.id}@callidusco.com`,
      `DTSTAMP:${stamp}`,
      `DTSTART;VALUE=DATE:${d}`,
      `DTEND;VALUE=DATE:${dn}`,
      `SUMMARY:${esc("Due: " + t.title + (t.entity_id ? " — " + entName(t.entity_id) : ""))}`,
      `DESCRIPTION:${esc("Task · Callidus Owner Portal — https://callidusco.com/administration")}`,
      "END:VEVENT",
    );
  }
  ...
  return new Response(lines.join("\r\n") + "\r\n", {
    headers: {
      ...
      "Content-Disposition": 'inline; filename="callidus-tasks.ics"',
