...
Deno.serve(async (req) => {
  ...
  const { data: owned } = await admin.from("ownership_entities").select("id").eq("id", entityId).eq("workspace_id", prof.workspace_id).maybeSingle();
  if (!owned) return json({ error: "Unknown unit" }, 404);
  ...
  try {
    const { data: t } = await admin.from("tenants").select("id").eq("workspace_id", prof.workspace_id).order("created_at").limit(1).maybeSingle();
    await admin.from("ai_usage").insert({
      workspace_id: prof.workspace_id, tenant_id: t?.id ?? null, model: "claude-opus-4-8",
