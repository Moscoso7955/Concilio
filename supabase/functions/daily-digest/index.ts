...
function digestHtml(name: string, today: Row[], overdue: Row[], entName: (id: string | null) => string) {
  ...
  <div style="background:#111111;padding:40px 16px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
    <div style="max-width:460px;margin:0 auto;background:#1a1a1a;border:1px solid #2f2f2f;border-radius:14px;padding:32px;text-align:center;">
      ...
      <a href="${PORTAL_URL}" style="display:inline-block;background:#b8903f;color:#111111;font-weight:600;font-size:15px;text-decoration:none;padding:12px 28px;border-radius:9px;margin-top:22px;">Open Management</a>
    </div>
  </div>`;
}
...
Deno.serve(async (req) => {
  ...
  // Cron path: shared key — every workspace. Portal path ("Send digest
  // now"): a workspace admin's JWT — only that workspace's tasks.
  ...
  if (!authed) {
    ...
    if (user?.id) {
      ...
      if (prof?.workspace_id) {
        ...
        authed = member?.role === "admin";
        onlyWs = prof.workspace_id;
      }
    }
  }
  ...
  const today = todayCT();
  let taskQ = admin.from("tasks").select("title,entity_id,due_date,assignee_email,status").neq("status", "done")
    .not("assignee_email", "is", null).lte("due_date", today).limit(500);
  if (onlyWs) taskQ = taskQ.eq("workspace_id", onlyWs);
