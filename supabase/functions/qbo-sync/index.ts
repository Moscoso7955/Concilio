...
Deno.serve(async (req) => {
  ...
  let balMonths = 0;
  try {
    balMonths = await syncBalances(conn.realm_id, tok.access_token, entityId, start, end);
  } catch (e) {
...
async function syncBalances(realmId: string, accessToken: string, entityId: string, start: string, end: string): Promise<number> {
  const rep = await fetch(
  ...
  for (const m of months) {
    ...
    rows.push({ entity_id: entityId, period: m.period, as_of: m.asOf, assets, liabilities, equity, detail: { lines }, synced_at: new Date().toISOString() });
  }
